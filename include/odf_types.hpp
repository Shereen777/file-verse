...
struct UserInfo {
    ...
    UserInfo(const string& user, const string& hash, UserRole r, uint32_t idx, uint64_t created)
        : role(r), user_index(idx), created_time(created), last_login(0), is_active(1) {
        strncpy(username, user.c_str(), sizeof(username) - 1);
        username[sizeof(username) - 1] = '\0';
        strncpy(password_hash, hash.c_str(), sizeof(password_hash) - 1);
...
struct FileEntry {
    ...
    FileEntry(const string& filename, EntryType entry_type, uint64_t file_size, 
              uint32_t perms, const string& file_owner, uint32_t file_inode)
        ...
        strncpy(owner, file_owner.c_str(), sizeof(owner) - 1);
...
struct FileMetadata {
    ...
    FileMetadata(const string& file_path, const FileEntry& file_entry)
        : entry(file_entry), blocks_used(0), actual_size(0) {
        strncpy(path, file_path.c_str(), sizeof(path) - 1);
...
struct SessionInfo {
    ...
    SessionInfo(const string& id, const UserInfo& user_info, uint64_t login)
